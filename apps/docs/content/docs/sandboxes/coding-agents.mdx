...
```bash
export TERMBRIDGE_SANDBOX_DAYTONA_AGENT_AUTO=true
```
...
```bash
export TERMBRIDGE_SANDBOX_DAYTONA_AGENT_AUTH_PATHS=~/.claude/.credentials.json,~/.codex/auth.json,~/.config/opencode/opencode.json
```

Need custom destinations inside the sandbox? Use explicit mappings:

```bash
export TERMBRIDGE_SANDBOX_DAYTONA_AGENT_AUTH_MAPS=~/.claude/.credentials.json=/home/daytona/.claude/.credentials.json
```
...
```bash
export TERMBRIDGE_SANDBOX_DAYTONA_AGENT_ENV=CUSTOM_TOKEN,ANOTHER_SECRET
```
...
```bash
export TERMBRIDGE_SANDBOX_DAYTONA_AGENT_INSTALL=false
```

To override the install packages entirely:

```bash
export TERMBRIDGE_SANDBOX_DAYTONA_AGENT_PACKAGES=@anthropic-ai/claude-code,@openai/codex,opencode
```

<Callout type="info">
  Auth sync copies local credentials into the sandbox. Only use this with sandboxes you trust, and prefer ephemeral sandboxes with `TERMBRIDGE_SANDBOX_DAYTONA_DELETE_ON_EXIT=true`.
</Callout>
