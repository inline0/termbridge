...
export TERMBRIDGE_SANDBOX_DAYTONA_REPO=https://github.com/inline0/termbridge-test-app.git
export TERMBRIDGE_SANDBOX_DAYTONA_BRANCH=main
export TERMBRIDGE_SANDBOX_DAYTONA_PATH=termbridge-test-app
export TERMBRIDGE_SANDBOX_DAYTONA_NAME=termbridge-sandbox
...
export TERMBRIDGE_SANDBOX_DAYTONA_SERVER_PORT=8080
```

You can still enable preview by setting `TERMBRIDGE_SANDBOX_DAYTONA_PREVIEW_PORT`; in direct mode it is forwarded as the server `--proxy` port.
...
| Variable | Purpose |
| --- | --- |
| `TERMBRIDGE_SANDBOX_DAYTONA_REPO` | Repo URL to clone (required) |
| `TERMBRIDGE_SANDBOX_DAYTONA_BRANCH` | Branch to check out (optional) |
| `TERMBRIDGE_SANDBOX_DAYTONA_PATH` | Path for the repo in the sandbox (optional) |
| `TERMBRIDGE_SANDBOX_DAYTONA_NAME` | Sandbox name (optional) |

If `TERMBRIDGE_SANDBOX_DAYTONA_PATH` is not set, Termbridge derives it from the repo name.
...
```bash
export TERMBRIDGE_SANDBOX_DAYTONA_GIT_USERNAME=your_github_username
export TERMBRIDGE_SANDBOX_DAYTONA_GIT_TOKEN=your_github_token
```

`TERMBRIDGE_SANDBOX_DAYTONA_GIT_PASSWORD` is also supported and overrides the token if set.
...
To show the preview view, set `TERMBRIDGE_SANDBOX_DAYTONA_PREVIEW_PORT` to the port your dev server uses inside the sandbox.
...
| --- | --- |
| `TERMBRIDGE_SANDBOX_DAYTONA_PUBLIC=true` | Creates a public sandbox |
| `TERMBRIDGE_SANDBOX_DAYTONA_PUBLIC=false` | Creates a private sandbox |
...
```bash
export TERMBRIDGE_SANDBOX_DAYTONA_NAME=termbridge-test-$(date +%s)
export TERMBRIDGE_SANDBOX_DAYTONA_DELETE_ON_EXIT=true
```
...
- Confirm `TERMBRIDGE_SANDBOX_DAYTONA_PREVIEW_PORT` matches the dev server port.

**Sandbox creation fails with quota errors**
- Delete old sandboxes in the Daytona dashboard.
- Use `TERMBRIDGE_SANDBOX_DAYTONA_DELETE_ON_EXIT=true` for short-lived sessions.
